          ...
          <Card key={index} className="transition-shadow duration-300 hover:shadow-lg" bodyStyle={{ padding: "16px" }}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <h3 className="text-gray-900 mb-4 text-lg font-medium">{child.name}</h3>
                <div className="space-y-2">{/* Render các trường khác trong INewProject */}</div>
              </div>

              <div className="ml-4 flex gap-2">
                <Tooltip title="Cập nhật">
                  <button className="rounded-full p-2 transition-colors duration-200 hover:bg-gray-100" onClick={() => onEdit && onEdit(child)}>
