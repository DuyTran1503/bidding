...
const Breadcrumb = () => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        className={clsx("text-m-medium cursor-pointer text-cyan-600")}
        onClick={() => {
          navigate("/dashboard");
        }}
      >
      ...
      {!!arrayPath.length && <FaCaretRight className="text-m-medium text-cyan-600" />}
      {arrayPath.map((path, index) => {
        const isActive = index === arrayPath.length - 1;
        return (
          <Fragment key={index}>
            <div
              className={clsx("text-m-medium cursor-pointer text-cyan-600", isActive && "cursor-default !text-gray-500")}
              onClick={() => {
            ...
            {!isActive && <FaCaretRight className="text-m-medium text-cyan-500" />}
          </Fragment>
        );
      })}
    </div>
