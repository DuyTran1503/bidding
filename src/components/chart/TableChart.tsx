...
const TableChart: React.FC<TableProps> = ({ compareData, projectId, valueType = "quantity", chartType = "bar" }) => {
  ...
  const rowClassName = (record: { id: string }) => (record.id == projectId ? "bg-red-300" : "");
      ...
      {chartType === "bar" && (
        <div className="mb-12 flex items-center justify-center space-x-4">
          <div className="flex items-center">
            <div className="mr-1 h-4 w-6 rounded-[4px] bg-[#5470C6]"></div>
            <span>Gói thầu bạn chọn so sánh</span>
          </div>
          <div className="flex items-center">
            <div className="mr-1 h-4 w-6 rounded-[4px] bg-[#FF0000]"></div>
            <span>Gói thầu mặc định</span>
          </div>
        </div>
      )}

      <Table columns={columns} dataSource={dataSource} pagination={false} rowClassName={rowClassName} bordered locale={{ emptyText }} />
    </>
  );
};
